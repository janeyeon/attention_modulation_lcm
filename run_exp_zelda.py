...
if __name__ == '__main__':
    ...
    parser.add_argument('--h_num', type=int)
    # name_list = {'
    ...
    def mod_forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, temb=None, config=None):
        ...
        if DENSEDIFF_ON and (COUNT < NUM_INFERENCE_STEPS*reg_part):
            ...
            #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                # print(f"mask: {torch.min(mask), torch.max(mask), torch.min(creg_maps[sim.size(1)]), torch.max(creg_maps[sim.size(1)]), mask.shape}")
    ...
    for hierar_step in range(h_num):
        if hierar_step == 0:
            # initial generation 
            ...
            config.img2img_input_path = config.output_path  / f'results_global_focus_{image_idx}_{config.seed}.png' # Later
        else:
            ...
            config.mix_ratio = 0.5
            config.reverse_ratio = 0.5
            config.img2img_input_path = config.output_path / f'results_group_focus_{image_idx}_{config.seed}.png' # Later
        ...
        with torch.no_grad():
            ...
            if config.use_img2img:
                ...
                latents = pipe.scheduler.add_noise(prior_image_latent, latents, timesteps=torch.tensor([t_mix], dtype=torch.long, device='cuda'))
