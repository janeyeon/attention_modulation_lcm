    ...
    with torch.no_grad():
        for i, group_idx in enumerate(group_ids):
            ...
            for j, inst_idx in enumerate(inst_ids):
                ...
                inst_obj_prompt_dic[j+1+prev_inst_points] = inst_prompt.replace('.','') #!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            for j, obj_idx in enumerate(obj_ids):
                ...
                inst_obj_prompt_dic[j+1001+prev_obj_points] = obj_prompt #!!!!!!!!!!!!!!!!!!!!!!!!
                if group_idx == 'non_group_person':
                    print('obj_prompt!!!!!!!!')
        ...
        pose_map_pil.save(config.output_path / f'Pose_{global_prompt}.png')        
        box_map.save(config.output_path / f'Boxes_{global_prompt}.png')
        pose_box_map = Image.fromarray(np.array(box_map) + pose_map)
        # display(pose_box_map)
        pose_box_map.save(config.output_path / f'PoseBoxes_{global_prompt}.png')
