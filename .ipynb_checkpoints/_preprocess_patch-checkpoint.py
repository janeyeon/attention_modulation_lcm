...
def preprocess_patch(pipe,
    ...
    with torch.no_grad():
        ...
        while condition:
            ...
            for k in range(1,len(prompts_for_view)):
                wlen = text_input['length'][k] - 2
                widx = text_input['input_ids'][k][1:1+wlen]
                for l in range(77):
                    if (text_input['input_ids'][0][l:l+wlen-1] == widx[:-1]).sum() == wlen-1:
                        pww_maps[:,l:l+wlen,:,:] = layouts[k-1:k]
                        cond_embeddings[0][l:l+wlen] = cond_embeddings[k][1:1+wlen]
                        # print(prompts_for_view[k], l, '-th segment is handled.')
                        break
            ...
            # full_prompt_for_views.append(full_prompt_for_view_multi)
            # prompts_for_views.append(prompts_for_view_multi)
            # sreg_maps_for_views.append(sreg_maps_for_view_multi)
            # reg_sizes_for_views.append(reg_sizes_for_view_multi)
            # creg_maps_for_views.append(creg_maps_for_view_multi)
            # text_cond_for_views.append(text_cond_for_view_multi)
            
        # print(f'exceed_cnt: {exceed_cnt}')
        # print(f'len(views): {len(views)}')

        # return full_prompt_for_views,\
        #         prompts_for_views,\
        #         text_cond_for_views,\
        #         sreg_maps_for_views,\
        #         reg_sizes_for_views,\
